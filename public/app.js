...
function route(){
  const view=++VIEW_ID,raw=(location.hash||"#/dashboard").replace(/^#\//,""),[r,q=""]=raw.split("?"),p=new URLSearchParams(q);
  ...
  Promise.resolve((map[r]||map.noaccess)()).catch(e=>{if(view!==VIEW_ID)return;toast(e.message,"error");const box=$("#content");if(box)box.innerHTML=`<div class="empty">${esc(e.message)}</div>`})
}
...
async function users(view=VIEW_ID){
  if(!has("accounts.manage"))return location.hash="#/"+homeRoute();const [u,i]=await Promise.all([req("/api/users"),req("/api/investors")]);if(!alive(view))return;const all=u.users||[],pending=all.filter(x=>x.account_status==="pending"),active=all.filter(x=>x.account_status==="active"),rejected=all.filter(x=>x.account_status==="rejected");
...
async function settings(view=VIEW_ID){
  if(!has("settings.read"))return location.hash="#/"+homeRoute();const x=await req("/api/settings");if(!alive(view))return;const s=x.settings||{};$("#content").innerHTML=head("設定","公司資料、運費及免運規則")+`<div class=card><form id=sf class=form-grid>
  ...
  if(has("settings.write"))$("#settingsSave").onclick=async()=>{const form=$("#sf"),f=Object.fromEntries(new FormData(form)),btn=$("#settingsSave"),status=$("#settingsStatus");f.default_delivery_cost_cents=cents(f.default_delivery_cost);f.customer_delivery_fee_cents=cents(f.customer_delivery_fee);f.free_shipping_threshold_cents=cents(f.free_shipping_threshold);btn.disabled=true;btn.textContent="儲存中…";status.textContent="正在儲存設定…";status.className="form-status";try{await req("/api/settings/save",{method:"POST",body:f});const fresh=await req("/api/settings");S.settings={...S.settings,...(fresh.settings||{})};status.textContent="已儲存";status.className="form-status success";toast("設定已儲存","success");shell();navigate("settings")}catch(er){console.error("SETTINGS_SAVE_FAILED",er);status.textContent=er.message||"儲存失敗";status.className="form-status error";btn.disabled=false;btn.textContent="儲存設定";toast(er.message||"儲存失敗","error")}}
}
