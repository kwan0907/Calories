...
async function investors(){
  if(!has("investors.read"))return location.hash="#/"+homeRoute();const a=has("investors.write"),x=await req("/api/investors");$("#content").innerHTML=head("投資者","投資比例及應佔利潤基礎",a?'<button id="add" class="btn primary">＋ 投資者</button>':"")+`<div class=card>${table(["名稱","比例","狀態","備註"],(x.investors||[]).map(i=>[esc(i.name),num(i.percentage)+"%",i.is_active?"啟用":"停用",esc(i.notes||"")]))}</div>`;if(a)$("#add").onclick=()=>investorModal(()=>investors())
}
...
function userModal(ins,after){const def="staff";modal("新增帳戶",`<form id=mf>${field("名稱","name")}${field("Email","email","email")}<div class=field>${roleOptions(def)}</div><div class=field>${sel("連結投資者","investor_id",[["","不連結"],...ins.map(i=>[i.id,i.name+" ("+num(i.percentage)+"%)"])],"")}</div>${field("密碼","password","password")}<div class=field><label>權限</label>${permsHtml(ROLE_DEFAULTS[def])}</div><button class="btn primary">建立</button></form>`);const f=$("#mf"),rs=f.querySelector('[name="access_role"]');wireRoleDefaults(f,rs);f.onsubmit=async e=>{e.preventDefault();const b=obj(e);b.permissions=collectPerms(f,b.access_role);try{await req("/api/users",{method:"POST",body:b});closeModal();after();toast("帳戶已建立","success")}catch(er){toast(er.message,"error")}}}
function manageUserModal(u,ins,after){const selected=u.permissions||ROLE_DEFAULTS[u.access_role]||[];modal(u.account_status==="pending"?"審批帳戶":"帳戶設定",`<form id=mf><div class="review-user"><b>${esc(u.name)}</b><span>${esc(u.email)}</span></div><div class=field>${sel("審批狀態","account_status",[["active","批准／啟用"],["pending","待審批"],["rejected","拒絕"]],u.account_status||"pending")}</div><div class=field>${roleOptions(u.access_role||"viewer")}</div><div class=field>${sel("連結投資者","investor_id",[["","不連結"],...ins.map(i=>[i.id,i.name+" ("+num(i.percentage)+"%)"])],u.investor_id||"")}</div><div class=field><label>權限</label>${permsHtml(selected)}</div><button class="btn primary">儲存審批／權限</button></form>`);const f=$("#mf"),rs=f.querySelector('[name="access_role"]');wireRoleDefaults(f,rs);f.onsubmit=async e=>{e.preventDefault();const b=obj(e);b.permissions=collectPerms(f,b.access_role);try{await req("/api/users/"+u.id,{method:"PATCH",body:b});closeModal();after();toast("帳戶設定已更新","success")}catch(er){toast(er.message,"error")}}}
