...
function investorModal(x,after){modal(x?"修改投資者":"新增投資者",`<form id=mf>${field("名稱","name","text",x?.name||"")}${field("比例 %","percentage","number",String(x?.percentage??0))}<div class=field>${sel("狀態","is_active",[["1","啟用"],["0","停用"]],String(x?.is_active??1))}</div><div class=field><label>備註</label><textarea name=notes>${esc(x?.notes||"")}</textarea></div><button class="btn primary">儲存</button></form>`);$("#mf").onsubmit=async e=>{e.preventDefault();const f=obj(e);f.percentage=+f.percentage||0;f.is_active=+f.is_active;try{await req(x?"/api/investors/"+x.id:"/api/investors",{method:x?"PATCH":"POST",body:f});closeModal();await after();toast(x?"投資者已更新":"投資者已新增","success")}catch(er){toast(er.message,"error")}}}
